 ...
 */
final class WPML_Network_Fields extends FWP_Settings_Section_Base_1x0x0
{
    ...
    protected function init()
    {
        $this->set_settings( array(
            'section_id'        => 'wpml-network-fields',
            'page_id'           => 'wpml-network-fields',
            'option_name'       => 'wpml-network-settings',
            'option_group'      => 'wpml-network-settings',
            'title'             => __( 'Multi Site Settings', 'wp-mailto-links' ),
            'fields'            => array(
                'capability' => array(
                    'label'         => __( 'Capability for individual sites:', 'wp-mailto-links' ),
                    'default_value' => 'manage_options',
                ),
                'default_settings_site' => array(
                    'label'         => __( 'Use the settings of this site as default for new sites:', 'wp-mailto-links' ),
                    'default_value' => '',
                ),
            ),
        ) );
    ...
    protected function show_capability( array $args )
    {
        $this->get_html_fields()->select(
            $args[ 'key' ]
            , array(
                'manage_options'    => __( 'Site Admins and Super Admins', 'wp-mailto-links' ),
                'manage_network'    => __( 'Only Super Admins', 'wp-mailto-links' ),
            )
        );
    }
    ...
    protected function show_default_settings_site( array $args )
    {
        ...
        $values = array();
        $values[ '' ] = __( '- none -', 'wp-mailto-links' );
