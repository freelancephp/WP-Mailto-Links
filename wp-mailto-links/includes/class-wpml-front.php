...
final class WPML_Front extends WPRun_Base_1x0x0
{
    ...
    /**
     * @var WPML_Settings_Page
     */
    private $settings_page = null;

    /**
     * Initialize
     * @param WPML_Settings_Page $settings_page
     */
    protected function init( WPML_Settings_Page $settings_page )
    {
     ...
     * @param WPML_Link $link
     */
    protected function set_link( WPML_Link $link )
    {
     ...
     * @param string $type
     */
    protected function apply_link_settings( WPML_Link $link, $type )
    {
        ...
        if ( $icon_type && ! ( $has_img && $no_icon_for_img ) && ! $link->has_attr_value( 'class', 'wpml-no-icon' ) ) {
            if ( 'dashicon' === $icon_type ) {
                $dashicon = $this->opt( 'icon_dashicon', $type );
                $icon = '<i class="wpml-icon dashicons-before '. $dashicon .'" aria-hidden="true"></i>';
            } else if ( 'fontawesome' === $icon_type ) {
                $fa = $this->opt( 'icon_fontawesome', $type );
                $icon = '<i class="wpml-icon fa '. $fa .'" aria-hidden="true"></i>';
            } else if ( 'image' === $icon_type ) {
                $image = $this->opt( 'icon_image', $type );
                $icon = '<span class="wpml-icon wpml-image wpml-icon-'. $image .'"></span>';
            }

            if ( 'left' === $this->opt( 'icon_position', $type ) ) {
                $link->add_to_attr( 'class', 'wpml-icon-left' );
                $link->set_content( $icon . $link->get_content() );
            } else if ( 'right' === $this->opt( 'icon_position', $type ) ) {
                $link->add_to_attr( 'class', 'wpml-icon-right' );
