...
final class Old_WPML_Front
{
    ...
    protected function createCustomFilterHooks()
    {
        if ($this->option->getValue('filter_body') || $this->option->getValue('filter_head')) {
            ...
            if (! Old_WPDev_Filter_FinalOutput::isCreated()) {
                Old_WPDev_Filter_FinalOutput::create();
            }
        }
        ...
        if (is_feed()) {
            ...
            if ($this->option->getValue('protect')) {
                wp_enqueue_script('wp-mailto-links',
                    Old_WPML::plugin()->getGlobal('pluginUrl') . '/js/wp-mailto-links.js',
                    array('jquery'),
                    WPML_VERSION);
            }
    ...
    public function filterWpHead()
    {
        ...
        if ($icon) {
            ...
            echo '.mail-icon-' . $icon . ' { background:url(' . Old_WPML::plugin()->getGlobal('pluginUrl') . '/images/mail-icon-' . $icon . '.png) no-repeat 100% 75%; padding-right:' . $padding . 'px; }';
        }
    ...
    public function filterRss($content)
    {
        ...
        $content = preg_replace($this->regexps['emailMailto'], 'mailto:'.Old_WPML::plugin()->__($this->option->getValue('protection_text')), $content);
        return $content;
    }
    ...
    {
        $emailReplacement = ($emailReplacement === null) ? Old_WPML::plugin()->__($this->option->getValue('protection_text')) : $emailReplacement;
    ...
    protected function createTemplateFunctions()
    {
        ...
        Old_WPML::plugin()->setGlobal('frontObject', $this);
        ...
        if (!function_exists('wpml_mailto')):
            function wpml_mailto($email, $display = null, $attrs = array())
            {
               ...
               return Old_WPML::plugin()->getGlobal('frontObject')->protectedMailto($display, $attrs);
            }
        endif;
        ...
        if (!function_exists('wpml_filter')):
            ...
            {
                return Old_WPML::plugin()->getGlobal('frontObject')->filterContent($content);
            }
        endif;
    }
