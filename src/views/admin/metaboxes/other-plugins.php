<h4><img src="<?php echo $plugin->getGlobal('pluginUrl') . '/images/icon-wp-external-links.png' ?>" width="16" height="16" /> <?php $plugin->_e('WP External Links') ?> -
    ...
    <?php elseif( file_exists( WP_PLUGIN_DIR . '/wp-external-links/wp-external-links.php')): ?>
        <a href="<?php echo get_bloginfo('url') ?>/wp-admin/plugins.php?plugin_status=inactive"><?php $plugin->_e('Activate') ?></a>
    <?php else: ?>
...
<p><?php $plugin->_e('Encode mailto links, email addresses or any text and hide them from spambots. Easy to use, plugin works directly when activated.') ?>
